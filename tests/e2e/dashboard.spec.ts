...
test('renders the finance review UI and all v0 sections', async ({ page }, testInfo) => {
  ...
  await expect(page.locator('.account-group').filter({ hasText: 'Credit and liabilities' }).locator('header')).toContainText('1 flagged');
  ...
  await expect(cashCalendar.locator('header')).toContainText('2 open / 3 logged');
